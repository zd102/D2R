...
```powershell
Start-Process .\D2R-Server-1.0.0-windows-x64-setup.exe -ArgumentList '/VERYSILENT /SUPPRESSMSGBOXES /NORESTART' -Wait
...
```bash
mkdir d2r-server
tar -xzf D2R-Server-1.0.0-linux-x64.tar.gz -C d2r-server
cd d2r-server
...
`main` 推送、Pull Request 和手动触发运行双平台测试、构建、打包及真实服务安装/升级检查；通过后的包可在 Actions artifacts 下载。推送与 `package.json` 版本一致的 `v版本号` 标签（例如 `v1.0.0`），两平台成功后自动创建 Release，附带安装包、本文档和 SHA-256 清单。失败不会发布只有一端产物的 Release。
