...
战斗关卡和隐藏区域还会按种子选择四种整体朝向，角色入口与 Boss 不再固定于同一相对方位。通路、房间、任务、宝箱、补给点、出口、地标和碰撞区域一同转向；旋转 90° 或 270° 时，矩形地图长宽互换，下表给出模板尺寸。角色入场与死亡复活使用当前地图的入口，营地入口保持固定。朝向使用独立随机流，不改变同种子生成的房间、怪群经验与掉落预算；连续两次出发仍可能随机到同一朝向。
