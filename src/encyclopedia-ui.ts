...
export class EncyclopediaScreen {
  ...
  private itemDetail(entry: EncyclopediaItem) {
    ...
    if (entry.rune) {
      ...
      body += statsList([['售价', POTIONS[entry.supply].kind === 'rejuvenation' ? '不可购买' : `${POTIONS[entry.supply].price} 金币`], ['获得方式', POTIONS[entry.supply].kind === 'rejuvenation' ? '怪物与宝箱低概率掉落' : '旅者补给 / 怪物与宝箱掉落'], ['效果', POTIONS[entry.supply].description], ['存放位置', '共享药水仓库']]);
    } else if (item) {
      if (entry.word) {
