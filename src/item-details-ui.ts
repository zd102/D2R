...
import { compatibleRunewords } from './runeword-crafting';
...
export function itemDetails(hero: HeroState, item: Item, options: { equipped?: boolean; editable?: boolean; showRanges?: boolean; socketEditorOpen?: boolean; comparison?: string; tooltip?: boolean } = {}) {
  ...
  const visibleItem = identified ? item : { ...item, mods: item.baseMods ?? {}, catalogId: undefined, affixes: undefined, runes: [], socketedJewels: [] };
  const mods = itemMods(visibleItem), facts: [string, string | number][] = [];
  ...
  if (item.ethereal) facts.push(['底材', '无形（无法修复）']);
  if (item.baseQuality === 'superior') facts.push(['底材品质', '超强']);
  ...
  const affixes = itemModifierLines(visibleItem);
  const recipes = options.editable && !options.equipped && !options.tooltip ? compatibleRunewords(item, hero.runes) : [];
  const recipePanel = recipes.length ? `<section class="item-runewords"><div class="section-label">可镶嵌的符文之语</div><p>点击名称查看属性；一键镶嵌将按顺序消耗所需符文。</p>${recipes.map(({ word, remaining, missing }) => `<div class="item-runeword-row">
    <button class="runeword-inspect" data-runeword="${escape(word.catalogId!)}" aria-label="${escape(word.name)} · 查看符文之语属性">${escape(word.name)}</button>
    <button class="secondary-button" data-craft-runeword="${escape(word.catalogId!)}" data-craft-item="${escape(item.id)}" ${missing.length ? 'disabled' : ''} aria-label="一键镶嵌${escape(word.name)}">一键镶嵌</button>
    <small>${item.runes?.length ? '补齐：' : ''}${remaining.map(rune => escape(runeLabel(rune))).join(' → ')}</small>
    <small class="${missing.length ? 'missing-runes' : 'positive'}">${missing.length ? `缺少：${missing.map(({ rune, count }) => `${escape(runeLabel(rune))} ×${count}`).join('、')}` : '符文齐备'}</small>
    </div>`).join('')}</section>` : '';
  return `<div class="item-detail-scroll ${options.showRanges ? 'show-ranges' : ''}"><header class="item-summary"><div class="item-showcase ${item.rarity}">${itemVisual(item)}</div><div><span class="rarity-tag ${item.rarity}">${rarityNames[item.rarity]} · ${kind}</span><h3 class="${item.rarity}">${escape(identified ? item.name : `未鉴定 ${item.base ?? item.name}`)}</h3>${item.base && item.base !== item.name ? `<small>${escape(item.base)}</small>` : ''}</div></header>
    ...
    ${identified || affixes.length ? `<section class="item-affix-panel">${!options.tooltip && affixes.some(line => line.range) ? `<label class="affix-range-toggle"><input type="checkbox" data-affix-ranges ${options.showRanges ? 'checked' : ''}/>显示词缀浮动范围</label>` : ''}<ul class="item-affixes">${affixes.map(line => `<li>${escape(line.text)}${line.range ? `<small class="affix-range">变量 ${escape(line.range)}</small>` : ''}</li>`).join('')}</ul></section>${identified && item.setId ? `<div class="set-bonuses">${escape(itemSetName(item) ?? '')}${item.setId === 'sigon' ? '<br>2 件：10% 生命偷取<br>3 件：100 防御<br>6 件：20 法力、12% 火抗、7 点物理减伤' : ''}</div>` : ''}` : '<p class="unidentified-note">鉴定后显示魔法属性</p>'}
    ${!identified && affixes.length ? '<p class="unidentified-note">鉴定后显示额外魔法属性</p>' : ''}${item.sockets ? `<section class="item-socket-panel"><div class="section-label">镶嵌 · ${filledSockets(item)} / ${item.sockets} 孔</div><div class="socket-row">${(item.runes ?? []).map(id => `<span class="socket-rune" data-tip="${escape(runeLabel(id))}">${runeArtwork(id)}<b>${runeNumber(id)}</b><small>${RUNES[id].name}</small></span>`).join('')}${(item.socketedJewels ?? []).map(jewel => `<span data-tip="${escape(jewel.name)}"><i data-lucide="gem"></i></span>`).join('')}${Array(Math.max(0, item.sockets - filledSockets(item))).fill('<span class="socket-empty">空</span>').join('')}</div>${options.editable && !options.equipped && identified && filledSockets(item) < item.sockets ? `<details class="socket-editor" ${options.socketEditorOpen ? 'open' : ''}><summary>镶嵌符文 / 珠宝</summary><div class="socket-options">${[...new Set(hero.runes)].map(id => `<button data-socket="${id}" aria-label="镶嵌${runeLabel(id)}">${runeArtwork(id)}<span>${RUNES[id].name}<small>${runeNumber(id)}</small></span></button>`).join('') || '<small>尚无符文</small>'}</div><div class="socket-options">${[...hero.inventory, ...hero.stash, ...hero.cube].filter(item => item.jewel && item.identified !== false).map(item => `<button data-socket-jewel="${escape(item.id)}"><i data-lucide="gem"></i>${escape(item.name)}</button>`).join('')}</div></details>` : ''}</section>` : ''}${recipePanel}${options.comparison ?? ''}</div>`;
}
