...
export function itemDetails(hero: HeroState, item: Item, options: { equipped?: boolean; editable?: boolean; showRanges?: boolean; socketEditorOpen?: boolean; comparison?: string } = {}) {
  ...
  return `<div class="item-detail-scroll ${options.showRanges ? 'show-ranges' : ''}"><header class="item-summary"><div class="item-showcase ${item.rarity}">${itemVisual(item)}</div><div><span class="rarity-tag ${item.rarity}">${rarityNames[item.rarity]} · ${kind}</span><h3 class="${item.rarity}">${escape(identified ? item.name : `未鉴定 ${item.base ?? item.name}`)}</h3>${item.base && item.base !== item.name ? `<small>${escape(item.base)}</small>` : ''}</div></header>
    ...
    ${identified ? `<section class="item-affix-panel">${affixes.some(line => line.range) ? `<label class="affix-range-toggle"><input type="checkbox" data-affix-ranges ${options.showRanges ? 'checked' : ''}/>显示词缀浮动范围</label>` : ''}<ul class="item-affixes">${affixes.map(line => `<li>${escape(line.text)}${line.range ? `<small class="affix-range">变量 ${escape(line.range)}</small>` : ''}</li>`).join('')}</ul></section>${item.setId ? `<div class="set-bonuses">${escape(itemSetName(item) ?? '')}${item.setId === 'sigon' ? '<br>2 件：10% 生命偷取<br>3 件：100 防御<br>6 件：20 法力、12% 火抗、7 点物理减伤' : ''}</div>` : ''}` : '<p class="unidentified-note">鉴定后显示魔法属性</p>'}
