export class GameAudio {
  ...
  play(kind: 'hit' | 'swing' | 'spell' | 'loot' | 'level' | 'hurt' | 'portal') {
    if (!this.context || !this.volume) return;
    const ctx = this.context, now = ctx.currentTime;
    const notes = kind === 'level' ? [330, 440, 554, 660] : kind === 'loot' ? [740, 980] : kind === 'spell' ? [180, 280, 420] : [kind === 'hit' ? 95 : kind === 'hurt' ? 65 : kind === 'portal' ? 220 : 160];
