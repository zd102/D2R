...
import { PLAYER_COUNTS, playerLifeFactor, playerExperienceFactor, playerDamageFactor, playerDropExponent } from './player-count';
...
export function settingsPanel(game: Game) {
  ...
  return `<div class="pause-layout"><section class="pause-journey"><div class="pause-intro"><img src="/sigil.svg" alt=""/><div><small>篝火尚温</small><h3>下一段旅程，整装待发</h3><p>你的冒险已暂停。</p></div></div>
    ...
    <div class="save-state"><i></i>${game.online ? game.onlineState !== 'ready' ? '在线连接已中断' : game.onlineSaveBusy ? '正在保存到服务器…' : `在线自动存档 · 最近保存 ${game.profile ? new Date(game.profile.updatedAt).toLocaleTimeString('zh-CN') : '—'}` : game.storageAvailable ? '本地自动存档已开启 · 每 8 秒保存' : '本地存档不可用'}</div>
    <button class="secondary-button" data-action="exit-mode">${game.online ? '保存并退出账号' : '保存并切换模式'}</button>
    <details class="control-guide"><summary>冒险者操作手册</summary><dl><dt>角色 / 技能 / 背包</dt><dd><kbd>C / T / I</kbd></dd><dt>任务 / 地图</dt><dd><kbd>J / Tab</kbd></dd><dt>药水快捷键</dt><dd><kbd>1 / 2 / 3 / 4</kbd> · 背包共享药水区配置</dd><dt>全部药水 / 功能药水</dt><dd>背包 → 共享药水 → 点击药水</dd><dt>交互 / 切换武器</dt><dd><kbd>F / X</kbd></dd><dt>跑步与行走</dt><dd><kbd>V</kbd></dd><dt>返回冒险</dt><dd><kbd>Esc</kbd></dd></dl></details></section>
    ...
    <p class="movement-hint" id="player-count-hint">生命 ×${playerLifeFactor(players)} · 经验 ×${Number(playerExperienceFactor(players).toFixed(2))} · 伤害 / 命中 ×${playerDamageFactor(players, game.hero.difficultyLevel)} · 掉落档位 ${playerDropExponent(players)} / 4<br>经验增幅低于生命增幅，适合能快速清怪的角色。3、5、7pp 减少不掉落概率，不增加寻获魔法装备。怪物强度与经验在生成时固定；掉落按击杀或开箱时设置结算。每个角色独立保存。</p>
