...
export class ProfileScreen {
  ...
  render() {
    ...
    if (ui.panel === 'profiles') {
      ...
      body = `<div class="roster-label"><span>${this.profiles.length} 位冒险者</span><span class="roster-storage">本地存档<button aria-label="导入角色存档" data-tip="导入角色存档" data-profile-action="import" ${game.saves && game.storageAvailable ? '' : 'disabled'}>${icon('upload')}</button></span></div>
