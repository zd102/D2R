...
export function supportsAffixProperty(code: string) {
  return Object.hasOwn(PROPERTY_MODS, code) || Object.hasOwn(PER_LEVEL, code) || ['charged', 'indestruct', 'skilltab', 'ease', 'cold-len', 'sock', 'dmg-pois', 'dmg-fire', 'dmg-cold', 'dmg-ltng'].includes(code);
}
...
export function affixName(affix: AffixDefinition) { const charge=affix.properties.find(p=>p[0]==='charged'), id=charge&&catalogSkill(String(charge[1])); if(id)return skillName(id); const mod=Object.keys(rollAffix(affix, () => 0).mods)[0] as Modifier;return NAMES[affix.name] ?? ({amazonSkills:'女武神',sorceressSkills:'术士',bowSkills:'弓术',javelinSkills:'枪术',passiveSkills:'猎手',fireSkillsTab:'火焰',coldSkills:'冰霜',lightningSkills:'闪电'} as Partial<Record<Modifier,string>>)[mod]?? MOD_LABELS[mod] ?? '秘法'; }
