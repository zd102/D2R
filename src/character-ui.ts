...
import { organizeItems, SLOTS, slotNames, rarityNames, MOD_NAMES, RUNES, AVAILABLE_RUNEWORDS as RUNEWORDS, packItems, footprint, stashRows, type Item, type Modifier, type RuneId } from './items';
...
export class CharacterScreen {
  ...
  constructor(game: Game, ui: UI) {
    ...
    ui.overlay.addEventListener('click', event => {
      ...
      if (data.action === 'sort-stash') { if (game.saveConflict || ui.sharedStashScreen.busy) return; changed = organizeItems(h.stash, stashRows(h.stash)); if (!changed) ui.toast('仓库空间不足，旧物品已保留，请先取出部分物品'); }
