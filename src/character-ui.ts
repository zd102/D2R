...
export class CharacterScreen {
  ...
  inventory() {
    ...
    return `<div class="inventory-screen" data-inventory-view="${this.view}" data-inventory-pane="${this.inventoryPane}">${tabs}<nav class="compact-tabs inventory-pane-tabs" aria-label="行囊视图">${(['items','equipment','details'] as const).map(pane => `<button data-inventory-pane="${pane}" aria-pressed="${this.inventoryPane === pane}" ${pane === 'details' && !selected ? 'disabled' : ''}>${({items:'物品',equipment:'装备',details:'详情'})[pane]}</button>`).join('')}</nav><div class="paladin-inventory" data-view="${this.view}"><div class="gear-column">${equipment}</div><div class="bag-column">${grid}<div class="utility-potions">${POTIONS.slice(2).map((potion, i) => `<button class="secondary-button" data-potion="${i + 2}" data-tip="${potion.description}" ${(h.potions[i + 2] ?? 0) === 0 ? 'disabled' : ''}>${icon(potion.icon)}${potion.name} · ${h.potions[i + 2] ?? 0}<kbd>${i + 3}</kbd></button>`).join('')}</div></div><div class="item-details ${this.showRanges ? 'show-ranges' : ''}"><div class="inventory-section-heading inventory-detail-heading"><span>物品详情</span><small>${selected ? equippedSlot ? '已穿戴' : inCube ? '赫拉迪克方块' : inStash ? '私人仓库' : '背包' : '尚未选择'}</small></div>${details}</div></div><footer class="inventory-footer"><span class="inventory-footer-hint">${this.view === 'runes' ? '符文按编号排列 · 齐备筛选仅检查符文材料' : '选择物品查看属性，拖至装备栏即可穿戴'}</span><div class="inventory-gold">${icon('coins')}<span>${h.gold.toLocaleString()}</span><small>金币</small></div>${this.view === 'runes' ? '' : `<button class="text-button" data-action="repair" ${h.gold >= repair && repair > 0 ? '' : 'disabled'}>${icon('wrench')}${repair ? `修理全部 · ${repair}` : '装备完好'}</button>`}</footer></div>`;
  }
}
