...
export class CharacterScreen {
  ...
  runes() {
    ...
    }).join('')}</div></section><section><div class="section-label recipe-heading"><span>符文之语 · ${recipes.length}</span><select data-recipe-filter="true" aria-label="符文之语筛选"><option value="all" ${this.recipeFilter === 'all' ? 'selected' : ''}>全部配方</option><option value="ready" ${this.recipeFilter === 'ready' ? 'selected' : ''}>符文齐备</option></select></div><div class="runeword-list">${recipes.map((word, index) => `<div ${Math.floor(index / 4) === this.recipePage ? '' : 'hidden'}><div class="runeword-heading"><strong>${escape(word.name)}</strong><span class="recipe-availability ${hasRunes(word) ? 'positive' : ''}">${hasRunes(word) ? '符文齐备' : '缺少符文'}</span></div><span class="rune-sequence">${word.runes.map(id => `<span>${RUNES[id].name}<small>${runeNumber(id)}</small></span>`).join('<i aria-hidden="true">›</i>')}</span><small class="runeword-base">${escape(runewordBaseLabel(word))} · ${word.runes.length} 孔</small></div>`).join('') || '<span class="muted">暂无匹配配方</span>'}</div><nav class="recipe-pagination" aria-label="配方分页"><button data-recipe-page="${this.recipePage - 1}" ${this.recipePage === 0 ? 'disabled' : ''}>上一页</button><span>${this.recipePage + 1} / ${pages}</span><button data-recipe-page="${this.recipePage + 1}" ${this.recipePage + 1 === pages ? 'disabled' : ''}>下一页</button></nav><p class="recipe-note">按顺序镶入对应的普通底材，孔数须与配方一致。</p></section></div>`;
  }
  inventory() {
