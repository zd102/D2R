...
import { SKILLS, skillById, skillValues, skillName, skillIcon, treeNames, attributeNames, FCR, FHR, FBR, isAura, type SkillTree, type SkillId, type ActionId, type Attribute } from './paladin';
...
import { WEAPON_TYPE_NAMES, itemRequirements } from './items';
...
const itemIcon = (item: Item) => item.charm ? 'scroll-text' : ({ weapon: 'sword', shield: 'shield', armor: 'shirt', helm: 'crown', gloves: 'hand', boots: 'footprints', belt: 'rectangle-ellipsis', amulet: 'gem', ring: 'circle', ring2: 'circle' } as const)[item.slot];
export const bindingNames: Record<Skill, string> = { attack: '主攻击', cleave: '快捷一', nova: '快捷二', dash: '快捷三', bolt: '副攻击' };
export class CharacterScreen {
  ...
  constructor(game: Game, ui: UI) {
    ...
    ui.overlay.addEventListener('click', event => {
      ...
      if (data.tree) { this.tree = data.tree as SkillTree; this.selectedSkill = SKILLS.find(skill => skill.tree === this.tree)!.id; render = true; }
      ...
      if (data.bagView) { this.view = data.bagView as typeof this.view; ui.selectedItem = undefined; render = true; }
      ...
      if (data.upgradeRune && !game.saveConflict) { changed = upgradeRune(h.runes, data.upgradeRune as RuneId); if (changed) ui.toast('符文已合成', RUNES[runeUpgradeCost(data.upgradeRune as RuneId)!.next].name); }
      ...
      if (data.action === 'confirm-respec') { changed = respec(h); this.respecPending = false; render = true; if (changed) { game.combat.zeal = null; ui.toast('属性与技能点已返还'); } }
    ...
    ui.overlay.addEventListener('change', event => {
      ...
      if (select.dataset.recipeFilter) { this.recipeFilter = select.value; ui.renderPanel(); }
  ...
  character() {
    const h = this.game.hero, s = stats(h), diff = difficulty(h), canRespec = h.questRewards.includes(`${diff}:shrine0`) && !h.respecUsed.includes(diff);
    const rows = [
      ['攻击伤害', `${Math.floor(s.attackMin)} - ${Math.floor(s.attackMax)}`], ['准确率', s.attackRating], ['防御', s.defense], ['格挡几率', `${s.block}%`],
      ['生命', `${Math.ceil(h.hp)} / ${number(s.maxHp)}`], ['法力', `${number(h.mana)} / ${number(s.maxMana)}`], ['耐力', `${Math.ceil(h.stamina)} / ${Math.ceil(s.maxStamina)}`], ['武器速度', `${s.attackFrames} 帧`],
    ];
    const next = (value: number, thresholds: number[]) => thresholds.find(threshold => threshold > value) ?? '已达上限';
    return `<div class="paladin-sheet"><div class="character-banner"><div class="class-mark">${icon('shield-check')}</div><div><span>圣骑士 · ${difficultyNames[diff]}</span><h3>${escape(this.game.profile?.name ?? '圣骑士')}</h3><small>等级 ${h.level} · 第 ${this.game.level.act + 1} 章 · 第 ${this.game.level.step + 1} 关</small></div></div>
      <div class="sheet-columns"><div><div class="section-label"><span>基础属性</span><span class="gold-text">剩余 ${h.points}</span></div><div class="attribute-list">${(Object.keys(attributeNames) as Attribute[]).map(key => `<div>${icon(({ strength: 'sword', dexterity: 'crosshair', vitality: 'heart', energy: 'droplets' })[key])}<span>${attributeNames[key]}<small>${h[key]} 基础${s[key] !== h[key] ? ` + ${s[key] - h[key]}` : ''}</small></span><b>${s[key]}</b><button data-allocate="${key}" ${tip(`提升${attributeNames[key]}`)} ${h.points ? '' : 'disabled'}>${icon('plus')}</button><button data-allocate="${key}" data-count="5" ${tip(`${attributeNames[key]}增加5点`)} ${h.points >= 5 ? '' : 'disabled'}>+5</button></div>`).join('')}</div>
  ...
  skillSummary(id: SkillId, rank: number) {
    ...
    if (v.min || v.max) { const multiplier = id === 'blessedHammer' && s.aura.id === 'concentration' ? 1 + s.aura.damage / 200 : 1; rows.push([`${damageTypeNames[v.type]}伤害`, `${Math.floor(v.min * multiplier)} - ${Math.floor(v.max * multiplier)}`]); }
    if (v.damage && v.type === 'physical') rows.push(['伤害加成', `+${v.damage}%`]);
    if (v.attack) rows.push(['准确率', `+${v.attack}%`]);
    if (v.hits > 1) rows.push(['连续攻击', `${v.hits} 次`]);
    if (v.duration) rows.push(['持续时间', `${number(v.duration)} 秒`]);
    if (v.healing) rows.push(['恢复生命', number(v.healing)]);
    ...
    if (isAura(id)) rows.push(['范围', `${number(v.radius)} 码`]);
    rows.push(['法力消耗', number(v.cost)]);
  ...
  skills() {
    ...
    const options = ['attack' as const, ...SKILLS.filter(skill => h.skills[skill.id]).map(skill => skill.id)];
    return `<div class="skill-toolbar"><div role="tablist" aria-label="技能系别" class="character-tabs">${(Object.keys(treeNames) as SkillTree[]).map(tree => `<button role="tab" aria-selected="${this.tree === tree}" data-tree="${tree}">${icon(tree === 'combat' ? 'swords' : tree === 'offensive' ? 'sun' : 'shield')}${treeNames[tree]}<b>${SKILLS.filter(skill => skill.tree === tree).reduce((sum, skill) => sum + h.skills[skill.id], 0)}</b></button>`).join('')}</div><span class="skill-points">剩余技能点 <b>${h.skillPoints}</b></span></div>
  ...
  runes() {
    ...
    return `<div class="rune-pouch">${(Object.keys(RUNES) as RuneId[]).map((id, index) => {
      const recipe = runeUpgradeCost(id), owned = count(id), title = recipe ? `${recipe.count} ${RUNES[id].name}合成 1 ${RUNES[recipe.next].name}` : '';
      return `<div class="rune-entry ${owned ? '' : 'muted'}">${icon('gem')}<strong>${RUNES[id].name}</strong><small>${index + 1} · ${id.toUpperCase()}</small><b>${owned}</b>${recipe ? `<button class="rune-upgrade" data-upgrade-rune="${id}" ${tip(title)} ${owned < recipe.count ? 'disabled' : ''}>${icon('arrow-up')}</button>` : ''}</div>`;
    }).join('')}</div><div class="section-label recipe-heading"><span>符文之语 · ${recipes.length}</span><select data-recipe-filter="true" aria-label="符文之语筛选"><option value="all" ${this.recipeFilter === 'all' ? 'selected' : ''}>全部配方</option><option value="ready" ${this.recipeFilter === 'ready' ? 'selected' : ''}>符文齐备</option></select></div><div class="runeword-list">${recipes.map(word => `<div><strong>${word.name}</strong><span>${word.runes.map(id => RUNES[id].name).join(' · ')}</span><small>${word.slots.map(slot => slot === 'weapon' && word.weaponTypes ? word.weaponTypes.map(type => WEAPON_TYPE_NAMES[type]).join(' / ') : slotNames[slot]).join(' / ')} · ${word.runes.length} 孔</small></div>`).join('') || '<span class="muted">暂无匹配配方</span>'}</div>`;
  }
  inventory() {
    ...
    const grid = this.view === 'runes' ? this.runes() : `<div class="section-label"><span>${this.view === 'stash' ? '私人仓库' : '背包'}</span><span>${this.view === 'stash' ? `${items.length} 件` : `${occupied} / 40 格`}</span></div><div class="diablo-grid" data-container="${this.view}" data-rows="${rows}" style="--rows:${rows}" aria-label="${this.view === 'stash' ? '仓库' : '背包'}物品">${items.map((item, i) => { const p = positions?.get(item.id) ?? { x: i % 10, y: Math.floor(i / 10), width: 1, height: 1 }; return `<button draggable="false" aria-roledescription="可移动装备" aria-keyshortcuts="Space Enter ArrowLeft ArrowRight ArrowUp ArrowDown Escape" class="bag-item ${item.rarity} ${item.id === this.ui.selectedItem ? 'selected' : ''}" style="grid-column:${p.x + 1}/span ${p.width};grid-row:${p.y + 1}/span ${p.height}" data-item="${escape(item.id)}" ${tip(item.identified === false ? `未鉴定 ${item.base ?? item.name}` : item.name)}>${icon(itemIcon(item))}${item.identified === false ? '<b class="unidentified-mark">?</b>' : ''}${item.sockets ? `<small>${item.runes?.length ?? 0}/${item.sockets}</small>` : ''}</button>`; }).join('')}</div><span class="inventory-move-status" role="status" aria-live="polite" aria-atomic="true"></span>`;
    let details = `<div class="empty-detail">${icon('shield-check')}<span>圣骑士装备</span><small>${active.length} 件生效 · 武器组 ${h.weaponSet + 1}</small></div>`;
    if (selected) {
      ...
      details = `<span class="rarity-tag ${selected.rarity}">${rarityNames[selected.rarity]} · ${selected.charm ? selected.charmSize ? CHARM_BASES[selected.charmSize].name : '护身符' : slotNames[selected.slot]}</span><div class="item-showcase ${selected.rarity}">${icon(itemIcon(selected))}</div><h3 class="${selected.rarity}">${escape(identified ? selected.name : `未鉴定 ${selected.base ?? selected.name}`)}</h3>${selected.base && selected.base !== selected.name ? `<small>${escape(selected.base)}</small>` : ''}
      <dl class="item-basics">${selected.slot === 'weapon' ? `<div><dt>${selected.twoHanded ? '双手' : '单手'}伤害</dt><dd>${itemWeaponDamage(selected, h.level, mods)}</dd></div>` : !selected.charm && !['ring', 'ring2', 'amulet'].includes(selected.slot) ? `<div><dt>防御</dt><dd>${Math.floor(selected.power * (1 + (mods.enhancedDefense ?? 0) / 100) + (mods.defense ?? 0) + (mods.defensePerLevel ?? 0) * h.level)}</dd></div>` : ''}${selected.block ? `<div><dt>盾牌格挡</dt><dd>${selected.block + (mods.block ?? 0)}%</dd></div>` : ''}${selected.maxDurability ? `<div><dt>耐久度</dt><dd>${mods.indestructible ? '无法破坏' : `${selected.durability} / ${selected.maxDurability}`}</dd></div>` : ''}<div><dt>物品等级</dt><dd>${selected.level}</dd></div><div><dt>需要等级</dt><dd>${selected.requiredLevel ?? 1}</dd></div>${selected.requiredStrength ? `<div><dt>需要力量</dt><dd>${identified ? itemRequirements(selected).strength : selected.requiredStrength}</dd></div>` : ''}${selected.requiredDexterity ? `<div><dt>需要敏捷</dt><dd>${identified ? itemRequirements(selected).dexterity : selected.requiredDexterity}</dd></div>` : ''}</dl>
      ${identified ? `<ul class="item-affixes">${itemModifierLines(selected).map(line => `<li>${escape(line.text)}${line.range ? `<small class="affix-range">变量 ${escape(line.range)}</small>` : ''}</li>`).join('')}</ul>${selected.setId ? '<div class="set-bonuses">西刚的全套刀剑<br>2 件：10% 生命偷取<br>3 件：100 防御<br>6 件：20 法力、12% 火抗、7 点物理减伤</div>' : ''}` : `<button class="primary-button" data-identify="${escape(selected.id)}" ${h.gold >= IDENTIFY_COST ? '' : 'disabled'}>${icon('scan-eye')}鉴定 · ${IDENTIFY_COST} 金币</button>`}
      ${selected.sockets ? `<div class="socket-row">${Array.from({ length: selected.sockets }, (_, i) => `<span>${selected.runes?.[i] ? RUNES[selected.runes[i]].name : '空'}</span>`).join('')}</div>${!slot && identified && (selected.runes?.length ?? 0) < selected.sockets ? `<label class="socket-label">镶嵌符文</label><div class="socket-options">${[...new Set(h.runes)].map(id => `<button data-socket="${id}" ${tip(`镶嵌${RUNES[id].name}`)}>${icon('gem')}${RUNES[id].name}</button>`).join('') || '<small>尚无符文</small>'}</div>` : ''}` : ''}${comparison}
      ${slot ? `<span class="equipped-label">已装备${!active.includes(selected) ? ' · 需求未满足' : ''}</span><button class="secondary-button" data-unequip="${slot}">${icon('backpack')}卸下</button>` : inStash ? `<button class="primary-button" data-withdraw="${escape(selected.id)}">${icon('arrow-down')}取回背包</button><button class="text-button" data-salvage="${escape(selected.id)}">${icon('coins')}出售 · ${selected.value}</button>` : `<button class="primary-button" data-equip="${escape(selected.id)}" ${reason ? 'disabled' : ''}>${icon('sword')}装备</button>${selected.slot === 'ring' ? `<button class="secondary-button" data-equip-ring="${escape(selected.id)}" ${reason ? 'disabled' : ''}>${icon('circle')}装备到右戒指</button>` : ''}<small class="learn-reason">${reason}</small><button class="secondary-button" data-stash="${escape(selected.id)}">${icon('archive')}存入仓库</button><button class="text-button" data-salvage="${escape(selected.id)}">${icon('coins')}出售 · ${selected.value}</button>`}`;
    }
    return `<div class="paladin-inventory"><div class="gear-column">${equipment}<div class="inventory-gold">${icon('coins')}${h.gold.toLocaleString()}</div></div><div class="bag-column">${tabs}${grid}<div class="bag-utilities"><button class="text-button" data-action="repair" ${h.gold >= repairCost(h) && repairCost(h) > 0 ? '' : 'disabled'}>${icon('wrench')}修理 ${repairCost(h)}</button></div></div><div class="item-details">${details}</div></div>`;
  }
}
