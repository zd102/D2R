...
export class CharacterScreen {
  ...
  constructor(game: Game, ui: UI) {
    ...
    ui.overlay.addEventListener('change', event => {
      ...
      if (select.dataset.potionBinding !== undefined && !game.saveConflict) {
        ...
        if (Number.isInteger(slot) && slot >= 0 && slot < 4 && POTIONS[index]) { game.hero.potionBindings[slot] = index; game.save(false); ui.renderPanel(); ui.update(0); ui.overlay.querySelector<HTMLSelectElement>(`[data-potion-binding="${slot}"]`)?.focus({ preventScroll: true }); }
      }
  ...
  private inventoryPotions() {
    const h = this.game.hero;
    const shortName = (index: number) => POTIONS[index].kind === 'health' || POTIONS[index].kind === 'mana'
      ? ['微型', '小型', '普通', '大型', '超级'][POTIONS[index].tier - 1]
      : ({ vps: '精力', yps: '解毒', wms: '溶解', rvs: '活力', rvl: '全面' })[POTIONS[index].code];
    const rows = [
      { name: '生命', kind: 'health', indices: [0, 5, 7, 9, 11] },
      { name: '法力', kind: 'mana', indices: [1, 6, 8, 10, 12] },
      { name: '特殊', kind: 'special', indices: [13, 14, 2, 3, 4] },
    ];
    return `<section class="inventory-potions" aria-label="共享药水与快捷键">
      <div class="inventory-potions-heading"><strong>${icon('flask-conical')}共享药水</strong><small>${this.game.online ? '同账号共享' : '本地角色共享'} · 点击使用</small></div>
      <div class="inventory-potion-bindings">${h.potionBindings.map((selected, slot) => `<label><kbd>${slot + 1}</kbd><select data-potion-binding="${slot}" aria-label="快捷键 ${slot + 1} 使用的药水">${POTIONS.map((potion, index) => `<option value="${index}" ${selected === index ? 'selected' : ''}>${potion.kind === 'health' || potion.kind === 'mana' ? `${shortName(index)}${potion.kind === 'health' ? '生命' : '法力'}` : potion.name.replace('恢复活力药剂', '活力').replace('药水', '')}</option>`).join('')}</select></label>`).join('')}</div>
      <div class="inventory-potion-stock">${rows.map(row => `<div class="inventory-potion-row" data-potion-kind="${row.kind}"><span>${row.name}</span>${row.indices.map(index => {
        const potion = POTIONS[index], count = h.potions[index] ?? 0;
        return `<button class="inventory-potion" data-potion="${index}" data-potion-code="${potion.code}" style="--potion-color:#${potion.color.toString(16)}" ${count ? '' : 'disabled'} ${tip(`${potion.name} · ${count.toLocaleString()} 瓶 · ${potionDescription(index, h.classId)} · 点击使用`)}><span>${shortName(index)}</span><b>${count.toLocaleString()}</b></button>`;
      }).join('')}</div>`).join('')}</div>
    </section>`;
  }
  inventory() {
