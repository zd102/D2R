...
export class CharacterScreen {
  game: Game; ui: UI;
  tree: SkillTree = 'combat'; selectedSkill: SkillId = 'sacrifice'; view: 'inventory' | 'stash' | 'runes' | 'potions' = 'inventory';
  respecPending = false;
  recipeFilter = 'all';
  recipePage = 0;
  showRanges = false;
  ...
  constructor(game: Game, ui: UI) {
    ...
    ui.overlay.addEventListener('click', event => {
      ...
      if (data.cubeWithdraw) { changed = transferItem(h, data.cubeWithdraw, 'inventory'); if (!changed) ui.toast('背包空间不足'); }
    ...
    ui.overlay.addEventListener('change', event => {
      ...
      if (select.dataset.potionBinding !== undefined && !game.saveConflict) {
        ...
        if (Number.isInteger(slot) && slot >= 0 && slot < 4 && POTIONS[index]) { game.hero.potionBindings[slot] = index; game.save(false); ui.renderPanel(); ui.update(0); }
      }
      ...
      if (select.dataset.binding) { const [kind, id, rank] = select.value.split(':'); if (kind === 'charged') bindChargedSkill(game.hero, select.dataset.binding as Skill, id as SkillId, Number(rank)); else bindSkill(game.hero, select.dataset.binding as Skill, select.value as ActionId); game.save(false); ui.renderPanel(); }
    });
  ...
  skills() {
    ...
    const options = ['attack' as const, ...ALL_SKILLS.filter(skill => skillLevel(h, skill.id)&&!isPassive(skill.id)).map(skill => skill.id)];
    return `<div class="skill-screen" data-skill-pane="${this.skillPane}"><nav class="compact-tabs skill-pane-tabs" aria-label="技能视图">${(['tree','detail','bindings'] as const).map(pane => `<button data-skill-pane="${pane}" aria-pressed="${this.skillPane === pane}">${({tree:'技能树',detail:'技能说明',bindings:'快捷配置'})[pane]}</button>`).join('')}</nav><div class="skill-toolbar"><div role="tablist" aria-label="技能系别" class="character-tabs">${trees.map(tree => `<button role="tab" aria-selected="${this.tree === tree}" data-tree="${tree}">${icon(SKILLS.find(skill=>skill.tree===tree)!.icon)}${treeNames[tree]}<b>${SKILLS.filter(skill => skill.tree === tree).reduce((sum, skill) => sum + h.skills[skill.id], 0)}</b></button>`).join('')}</div><span class="skill-points">剩余技能点 <b>${h.skillPoints}</b></span></div>
      ...
      <div class="section-label binding-heading"><span>技能配置</span><span>${h.classId==='paladin'?h.activeAura ? `当前灵气 · ${skillName(h.activeAura)}` : '无激活灵气':'被动技能自动生效'}</span></div><div class="binding-grid">${(Object.keys(h.bindings) as Skill[]).map(key => `<label><span>${bindingNames[key]}</span><select data-binding="${key}" aria-label="${bindingNames[key]}">${options.filter(id => key !== 'attack' || !isAura(id)).map(id => `<option value="${id}" ${h.bindings[key] === id && !h.chargeBindings?.[key] ? 'selected' : ''}>${skillName(id)}</option>`).join('')}${availableCharges(h).map(charge => `<option value="charged:${charge.id}:${charge.rank}" ${h.chargeBindings?.[key]?.id === charge.id && h.chargeBindings[key]?.rank === charge.rank ? 'selected' : ''}>${skillName(charge.id)} · 等级 ${charge.rank} 聚气 ${charge.remaining}/${charge.maximum}</option>`).join('')}</select></label>`).join('')}</div></div>`;
  }
  runes() {
    const runes = this.game.hero.runes, count = (id: RuneId) => runes.filter(r => r === id).length;
    const hasRunes = (word: (typeof RUNEWORDS)[number]) => word.runes.every(id => count(id) >= word.runes.filter(r => r === id).length);
    const recipes = RUNEWORDS.filter(word => this.recipeFilter !== 'ready' || hasRunes(word));
    const pages = Math.max(1, Math.ceil(recipes.length / 4)); this.recipePage = Math.min(this.recipePage, pages - 1);
    return `<nav class="compact-tabs rune-pane-tabs" aria-label="符文视图"><button data-rune-pane="materials" aria-pressed="${this.runePane === 'materials'}">符文材料</button><button data-rune-pane="recipes" aria-pressed="${this.runePane === 'recipes'}">符文之语</button></nav><div class="rune-layout" data-rune-pane="${this.runePane}"><section><div class="section-label inventory-section-heading"><span>符文材料</span><span>持有 ${RUNE_ORDER.filter(id => count(id) > 0).length} / 33 种</span></div><div class="rune-pouch">${RUNE_ORDER.map(id => {
    ...
    }).join('')}</div></section><section><div class="section-label recipe-heading"><span>符文之语 · ${recipes.length}</span><select data-recipe-filter="true" aria-label="符文之语筛选"><option value="all" ${this.recipeFilter === 'all' ? 'selected' : ''}>全部配方</option><option value="ready" ${this.recipeFilter === 'ready' ? 'selected' : ''}>符文齐备</option></select></div><div class="runeword-list">${recipes.map((word, index) => `<div data-runeword="${escape(word.catalogId!)}" tabindex="0" role="button" aria-label="${escape(word.name)} · 查看符文之语属性" ${Math.floor(index / 4) === this.recipePage ? '' : 'hidden'}><div class="runeword-heading"><strong>${escape(word.name)}</strong><span class="recipe-availability ${hasRunes(word) ? 'positive' : ''}">${hasRunes(word) ? '符文齐备' : '缺少符文'}</span></div><span class="rune-sequence">${word.runes.map(id => `<span>${RUNES[id].name}<small>${runeNumber(id)}</small></span>`).join('<i aria-hidden="true">›</i>')}</span><small class="runeword-base">${escape(runewordBaseLabel(word))} · ${word.runes.length} 孔</small></div>`).join('') || '<span class="muted">暂无匹配配方</span>'}</div><nav class="recipe-pagination" aria-label="配方分页"><button data-recipe-page="${this.recipePage - 1}" ${this.recipePage === 0 ? 'disabled' : ''}>上一页</button><span>${this.recipePage + 1} / ${pages}</span><button data-recipe-page="${this.recipePage + 1}" ${this.recipePage + 1 === pages ? 'disabled' : ''}>下一页</button></nav><p class="recipe-note">按顺序镶入对应的普通底材，孔数须与配方一致。</p></section></div>`;
  }
  inventory() {
    ...
    const equippedSlot = SLOTS.find(slot => h.equipment[slot]?.id === selected?.id), inStash = selected && h.stash.includes(selected), inCube = selected && h.cube.includes(selected);
    ...
    const tabs = `<div class="character-tabs bag-tabs inventory-primary-tabs" role="tablist" aria-label="物品容器">${(['inventory', 'stash', 'runes', 'potions'] as const).map(view => `<button role="tab" aria-selected="${this.view === view}" data-bag-view="${view}">${icon(view === 'inventory' ? 'backpack' : view === 'stash' ? 'archive' : view === 'potions' ? 'flask-conical' : 'gem')}${({ inventory: '背包', stash: '私人仓库', runes: '符文', potions: '药水' })[view]}</button>`).join('')}</div>`;
    if (this.view === 'potions') return `<div class="inventory-screen">${tabs}<section class="potion-storage"><p>${this.game.online ? '同一账号所有角色共享药水' : '本地所有角色共享药水'} · 快捷键配置随角色保存</p><div class="potion-bindings">${h.potionBindings.map((selected, slot) => `<label><span>快捷键 ${slot + 1}</span><select data-potion-binding="${slot}" aria-label="快捷键 ${slot + 1} 使用的药水">${POTIONS.map((potion, index) => `<option value="${index}" ${selected === index ? 'selected' : ''}>${potion.name} · ${h.potions[index] ?? 0}</option>`).join('')}</select></label>`).join('')}</div><div class="potion-storage-grid">${['health', 'mana', 'rejuvenation', 'utility'].map(kind => `<section><h3>${({health:'生命药水',mana:'法力药水',rejuvenation:'活力药剂 · 仅掉落',utility:'功能药水'})[kind]}</h3>${POTIONS.map((potion, index) => potion.kind !== kind ? '' : `<div class="potion-storage-entry" data-potion-code="${potion.code}"><span>${icon(potion.icon)}</span><div><strong>${potion.name}</strong><small>${potionDescription(index, h.classId)}</small></div><b>${(h.potions[index] ?? 0).toLocaleString()}</b><button class="secondary-button" data-potion="${index}" ${(h.potions[index] ?? 0) > 0 ? '' : 'disabled'} aria-label="使用${potion.name}">使用</button></div>`).join('')}</section>`).join('')}</div></section></div>`;
    const drawGrid = (container: ItemContainer) => {
      ...
      return `<section class="inventory-container" data-storage-section="${container}"><div class="section-label inventory-section-heading"><span>${container === 'cube' ? icon('box') : ''}${name}</span><span>${container === 'stash' ? `${items.length} 件` : `${occupied} / ${rows * columns} 格`}</span></div><div class="diablo-grid" data-container="${container}" data-rows="${rows}" data-columns="${columns}" style="--rows:${rows};--columns:${columns}" aria-label="${name}">${items.map((item, i) => {
      ...
      }).join('')}</div>${container === 'cube' ? '<p class="cube-storage-note">可与背包拖拽交换 · 方块内护符不生效</p>' : ''}</section>`;
    };
    const grid = this.view === 'runes' ? this.runes() : `<div class="inventory-grid-scroll">${drawGrid(this.view)}${this.view === 'inventory' && hasCube(h) ? drawGrid('cube') : ''}</div><p class="inventory-storage-note">${this.view === 'stash' ? '存放备用装备 · 向下滚动查看所有物品' : '随身携带 · 拖动物品整理或交换位置'}</p><span class="inventory-move-status" role="status" aria-live="polite" aria-atomic="true"></span>`;
    ...
    if (selected) {
      ...
      let comparison = '';
      if (!slot && identified && !selected.charm && !reason) { const clone = structuredClone(h); if (equipItem(clone, selected.id, undefined, inCube ? 'cube' : inStash ? 'stash' : 'inventory')) { const current = stats(h), next = stats(clone); comparison = `<div class="item-comparison">${([['伤害', next.attack - current.attack], ['防御', next.defense - current.defense], ['生命', next.maxHp - current.maxHp], ['格挡', next.block - current.block]] as [string, number][]).filter(([, value]) => Math.abs(value) > .01).map(([label, value]) => `<span class="${value > 0 ? 'positive' : 'negative'}">${label} ${value > 0 ? '+' : ''}${number(value)}</span>`).join('')}</div>`; } }
    ...
    return `<div class="inventory-screen" data-inventory-view="${this.view}" data-inventory-pane="${this.inventoryPane}">${tabs}<nav class="compact-tabs inventory-pane-tabs" aria-label="行囊视图">${(['items','equipment','details'] as const).map(pane => `<button data-inventory-pane="${pane}" aria-pressed="${this.inventoryPane === pane}" ${pane === 'details' && !selected ? 'disabled' : ''}>${({items:'物品',equipment:'装备',details:'详情'})[pane]}</button>`).join('')}</nav><div class="paladin-inventory" data-view="${this.view}"><div class="gear-column">${equipment}</div><div class="bag-column">${grid}<button class="secondary-button potion-storage-link" data-bag-view="potions">${icon('flask-conical')}共享药水 · ${h.potions.reduce((sum, count) => sum + count, 0).toLocaleString()} 瓶 · 配置快捷键</button></div><div class="item-details ${this.showRanges ? 'show-ranges' : ''}"><div class="inventory-section-heading inventory-detail-heading"><span>物品详情</span><small>${selected ? equippedSlot ? '已穿戴' : inCube ? '赫拉迪克方块' : inStash ? '私人仓库' : '背包' : '尚未选择'}</small></div>${details}</div></div><footer class="inventory-footer"><span class="inventory-footer-hint">${sharedScope} &middot; ${this.view === 'runes' ? '符文按编号排列 · 齐备筛选仅检查符文材料' : '选择物品查看属性，拖至装备栏即可穿戴'}</span><div class="inventory-gold">${icon('coins')}<span>${h.gold.toLocaleString()}</span><small>金币</small></div>${this.view === 'runes' ? '' : `<button class="text-button" data-action="repair" ${h.gold >= repair && repair > 0 ? '' : 'disabled'}>${icon('wrench')}${repair ? `修理全部 · ${repair}` : '装备完好'}</button>`}</footer></div>`;
  }
}
