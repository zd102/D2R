...
import { rangedBase, quantityLeft, maxQuantity } from './items';
import { useAmmoBundle, AMMO_BUNDLE } from './ranged';
...
export class CharacterScreen {
  ...
  constructor(game: Game, ui: UI) {
    ...
    ui.overlay.addEventListener('click', event => {
      ...
      if (data.useAmmo && !game.saveConflict) { changed = useAmmoBundle(h, data.useAmmo); if (changed) ui.selectedItem = undefined; else ui.toast('弹药储备已满'); }
  ...
  character() {
    ...
    return `<div class="paladin-sheet"><div class="character-banner"><div class="class-mark" style="color:${c.color}">${icon(c.icon)}</div><div><span>${c.name} · ${difficultyNames[diff]}</span><h3>${escape(this.game.profile?.name ?? c.name)}</h3><small>等级 ${h.level} · 第 ${this.game.level.act + 1} 章 · 第 ${this.game.level.step + 1} 关</small></div></div>
  ...
  runes() {
    ...
    return `<div class="rune-pouch">${RUNE_ORDER.map(id => {
  ...
  inventory() {
    ...
    if (selected) {
      ...
      details = `<span class="rarity-tag ${selected.rarity}">${rarityNames[selected.rarity]} · ${selected.misc ? '杂物' : selected.charm ? selected.charmSize ? CHARM_BASES[selected.charmSize].name : '护身符' : selected.jewel ? '珠宝' : slotNames[selected.slot]}</span><div class="item-showcase ${selected.rarity}">${itemVisual(selected)}</div><h3 class="${selected.rarity}">${escape(identified ? selected.name : `未鉴定 ${selected.base ?? selected.name}`)}</h3>${selected.base && selected.base !== selected.name ? `<small>${escape(selected.base)}</small>` : ''}
      <dl class="item-basics">${selected.slot === 'weapon' ? `<div><dt>${rangedBase(selected)?.stack ? '投掷' : selected.twoHanded ? '双手' : '单手'}伤害</dt><dd>${itemWeaponDamage(selected, h.level, mods)}</dd></div>` : !selected.charm && !['ring', 'ring2', 'amulet'].includes(selected.slot) ? `<div><dt>防御</dt><dd>${Math.floor(selected.power * (1 + (mods.enhancedDefense ?? 0) / 100) + (mods.defense ?? 0) + (mods.defensePerLevel ?? 0) * h.level)}</dd></div>` : ''}${selected.block ? `<div><dt>盾牌格挡</dt><dd>${selected.block + (mods.block ?? 0)}%</dd></div>` : ''}${selected.maxDurability ? `<div><dt>耐久度</dt><dd>${mods.indestructible ? '无法破坏' : `${selected.durability} / ${selected.maxDurability}`}</dd></div>` : ''}${maxQuantity(selected) ? `<div><dt>投掷数量</dt><dd>${quantityLeft(selected)} / ${maxQuantity(selected)}</dd></div>` : ''}<div><dt>物品等级</dt><dd>${selected.level}</dd></div><div><dt>需要等级</dt><dd>${selected.requiredLevel ?? 1}</dd></div>${selected.requiredStrength ? `<div><dt>需要力量</dt><dd>${identified ? itemRequirements(selected).strength : selected.requiredStrength}</dd></div>` : ''}${selected.requiredDexterity ? `<div><dt>需要敏捷</dt><dd>${identified ? itemRequirements(selected).dexterity : selected.requiredDexterity}</dd></div>` : ''}</dl>
      ${selected.requiredClass ? `<small>仅限${CLASS_NAMES[selected.requiredClass]}</small>` : ''}
      ${identified ? `<ul class="item-affixes">${itemModifierLines(selected).map(line => `<li>${escape(line.text)}${line.range ? `<small class="affix-range">变量 ${escape(line.range)}</small>` : ''}</li>`).join('')}</ul>${selected.setId ? `<div class="set-bonuses">${escape(itemSetName(selected) ?? '')}${selected.setId === 'sigon' ? '<br>2 件：10% 生命偷取<br>3 件：100 防御<br>6 件：20 法力、12% 火抗、7 点物理减伤' : ''}</div>` : ''}` : `<button class="primary-button" data-identify="${escape(selected.id)}" ${h.gold >= IDENTIFY_COST ? '' : 'disabled'}>${icon('scan-eye')}鉴定 · ${IDENTIFY_COST} 金币</button>`}
      ${selected.sockets ? `<div class="socket-row">${(selected.runes ?? []).map(id => `<span class="socket-rune" ${tip(runeLabel(id))}>${runeArtwork(id)}<b>${runeNumber(id)}</b><small>${RUNES[id].name}</small></span>`).join('')}${(selected.socketedJewels ?? []).map(jewel => `<span ${tip(jewel.name)}>${icon('gem')}</span>`).join('')}${Array(Math.max(0, selected.sockets - filledSockets(selected))).fill('<span class="socket-empty">空</span>').join('')}</div>${!slot && identified && filledSockets(selected) < selected.sockets ? `<label class="socket-label">镶嵌符文</label><div class="socket-options">${[...new Set(h.runes)].map(id => `<button data-socket="${id}" ${tip(`镶嵌${runeLabel(id)}`)}>${runeArtwork(id)}<span>${RUNES[id].name}<small>${runeNumber(id)}</small></span></button>`).join('') || '<small>尚无符文</small>'}</div><div class="socket-options">${[...h.inventory, ...h.stash].filter(item => item.jewel && item.identified !== false).map(item => `<button data-socket-jewel="${escape(item.id)}" ${tip(`镶嵌${item.name}`)}>${icon('gem')}${escape(item.name)}</button>`).join('')}</div>` : ''}` : ''}${comparison}
