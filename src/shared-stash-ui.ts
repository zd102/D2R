...
export class SharedStashScreen {
  ...
  constructor(ui: UI) {
    ...
    ui.overlay.addEventListener('click', event => {
      ...
      if (button.dataset.sharedPane) { this.pane = button.dataset.sharedPane as typeof this.pane; this.selected = undefined; ui.renderPanel(); }
      if (button.dataset.sharedItem) { this.selected = { side: button.dataset.sharedSide as 'personal' | 'shared', id: button.dataset.sharedItem }; ui.renderPanel(); }
  ...
  render() {
    ...
    return `<div class="shared-intro"><i data-lucide="archive"></i><p>本地角色共用 · 选择物品后存入或取出<small>同一浏览器、同一访问地址共享，容量 ${SHARED_STASH_ROWS * 10} 格。</small></p></div>
      ${this.error ? `<p class="shared-error" role="alert">${escape(this.error)}</p>` : ''}<div class="shared-workspace" data-pane="${this.pane}"><nav class="shared-mobile-tabs" aria-label="仓库视图"><button data-shared-pane="personal" aria-pressed="${this.pane === 'personal'}">个人物品</button><button data-shared-pane="shared" aria-pressed="${this.pane === 'shared'}">共享仓库</button></nav><div class="shared-columns"><section><div class="shared-tabs" role="tablist" aria-label="个人物品来源">${(['inventory', 'stash'] as const).map(view => `<button role="tab" aria-selected="${view === this.view}" data-shared-view="${view}" ${disabled ? 'disabled' : ''}>${view === 'inventory' ? '背包' : '个人仓库'} <small>${this.ui.game.hero[view].length}</small></button>`).join('')}</div>${this.grid(personal, this.view === 'inventory' ? 4 : stashRows(personal), 'personal')}<p class="shared-caption">${this.view === 'inventory' ? '背包' : '个人仓库'} → 共享仓库</p></section>
