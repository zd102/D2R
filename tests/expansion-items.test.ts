...
test('all 33 runes and the full LoD catalog remain, with 87 recipes craftable by seven playable classes', () => {
  assert.equal(RUNE_ORDER.length, 33); assert.deepEqual(Object.keys(RUNES), [...RUNE_ORDER]);
  assert.equal(RUNEWORDS.length, 87); assert.ok(BASES.length >= 500); assert.equal(SPECIAL_ITEMS.length, 506);
  ...
  assert.equal(AVAILABLE_RUNEWORDS.length, 87);
