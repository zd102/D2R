...
test('level rewards cross multiple thresholds and fully recover resources', () => {
  ...
  assert.equal(gainXp(hero, 230), true);
  assert.equal(hero.level, 3); assert.equal(hero.xp, 5); assert.equal(hero.points, 6);
...
test('equipping swaps the previous item into the bag without deleting equipment', () => {
  const hero = newHero(); const item = { ...rollItem(2, .8), slot: 'weapon' as const };
...
test('invalid and malicious saves cannot inject item markup or invalid stats', () => {
  ...
  assert.equal(loaded?.level, 1); assert.equal(loaded?.hp, 1); assert.equal(loaded?.mana, 100);
  assert.deepEqual(loaded?.shrines, [0]); assert.deepEqual(loaded?.inventory, []);
  assert.equal(loaded?.equipment.weapon?.id, 'starter-sword');
});
test('boss rewards always use legendary rarity', () => {
  for (let i = 0; i < 20; i++) assert.equal(rollItem(4, 0, true).rarity, 'legendary');
});
