...
try {
  ...
  await page.screenshot({ path: '.verification/paladin-character-desktop.png' });
  ...
  await page.screenshot({ path: '.verification/paladin-skills-desktop.png' });
  ...
  await page.screenshot({ path: '.verification/paladin-hammer-desktop.png' });
  ...
  for (const viewport of [{ width: 390, height: 844 }, { width: 360, height: 640 }, { width: 844, height: 390 }]) {
    ...
    await mobile.screenshot({ path: `.verification/paladin-game-${viewport.width}.png` });
    ...
    for (const panel of ['character', 'skills', 'inventory']) {
      ...
      await mobile.screenshot({ path: `.verification/paladin-${panel}-${viewport.width}.png` });
