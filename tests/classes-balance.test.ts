...
function duel(level:number,difficulty:0|1|2,index:number,build:'fire'|'cold'|'lightning'|'bow'|'javelin', beginCombat:()=>void){
  ...
  // Three.js UUID allocation during model construction must not choose combat rolls.
  beginCombat();
...
test('legal Amazon and Sorceress builds can damage and defeat representative chapter bosses within bounded mana budgets',t=>{
  ...
  for(const [level,diff,index] of [[12,0,4],[41,0,24],[67,1,24],[89,2,24]] as const)for(const build of ['fire','cold','lightning','bow','javelin'] as const){const r=duel(level,diff,index,build,()=>{seed=99;});rows.push(r);}
  console.table(rows);
