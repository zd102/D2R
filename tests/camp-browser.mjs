...
async function loadCamp(page, creating = false) {
  ...
  assert.deepEqual(s.objectives.map(p => p.kind), ['camp-portal', 'supply', 'base-merchant', ...(s.campaign.cleared.some(count => count >= 5) ? ['mercenary-merchant'] : []), ...(s.campaign.cleared[0] >= 25 ? ['socket-merchant'] : [])]);
