...
import {parseBaseStock} from '../src/progression-equipment.ts';
...
try {
  for(const width of [1440,390]) {
    ...
    hero.baseStock=parseBaseStock({difficulty:0,offers:[['insight-bow','8hb',4],['utility-helm','msk',3],['armor','lea',2],['sword','ssd',2],['cap','cap',2]].map(([id,code,sockets])=>({id,code,sockets,sold:false}))});
    ...
    await page.locator('[data-action="base-merchant"]').click();
    await expect(page.locator('.panel-base-shop')).toBeVisible();
    await expect(page.locator('[data-buy-base]')).toHaveCount(5);
    ...
    assert.equal(bought.gold,20000-hero.baseStock.offers[0].price-hero.baseStock.offers[1].price);
    await expect(page.locator('[data-buy-base="insight-bow"]')).toBeDisabled();
    await expect(page.locator('[data-buy-base="utility-helm"]')).toHaveText('已售罄');const bow=bought.inventory.find(item=>item.baseCode==='8hb'),helm=bought.inventory.find(item=>item.baseCode==='msk');
    ...
    await expect(page.locator('[data-buy-base]')).toHaveCount(5);
    await expect(page.locator('[data-buy-base="insight-bow"]')).toBeDisabled();
    assert.deepEqual((await savedProfile(page)).hero.baseStock,bought.baseStock);
