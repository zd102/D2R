...
import { BASES, SPECIAL_ITEMS, RUNE_ORDER } from '../src/items.ts';
...
try {
  ...
  for (const viewport of [{ width: 1440, height: 960 }, { width: 390, height: 844 }, { width: 360, height: 640 }]) {
    ...
    await expect(page.locator('.rune-entry')).toHaveCount(33); await expect(page.locator('.runeword-list>div')).toHaveCount(25);
    ...
    await page.locator('[data-recipe-filter]').selectOption('ready'); assert.ok(await page.locator('.runeword-list>div').count() < 25);
